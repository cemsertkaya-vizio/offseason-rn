...
type WalkingDistanceScreenNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  'WalkingDistance'
>;

interface WalkingDistanceScreenProps {
...
export const WalkingDistanceScreen: React.FC<WalkingDistanceScreenProps> = ({
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      ...
      console.log('WalkingDistanceScreen - Loading saved data for user:', user.id);
      const result = await profileService.getOnboardingStatus(user.id);

      if (result.success && result.profile?.onboarding_data?.walking?.distance) {
        console.log('WalkingDistanceScreen - Pre-filling with saved distance');
        const distanceData = result.profile.onboarding_data.walking.distance;
  ...
  const handleNext = async () => {
    ...
    setIsSaving(true);
    console.log('WalkingDistanceScreen - Saving progress with distance:', distanceData);
    ...
    if (!saveResult.success) {
      setIsSaving(false);
      console.log('WalkingDistanceScreen - Error saving progress:', saveResult.error);
      Alert.alert(
        'Error',
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.content, styles.scrollContent]}>
        ...
        <Text style={styles.question}>
          How far or long do you walk or hike?
        </Text>
