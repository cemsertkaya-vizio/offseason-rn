...
type RunningScreenNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  'Running'
>;
...
type RunningLocation = 'track' | 'pavement' | 'treadmill' | 'trail' | null;

const OPTIONS = [
  { id: 'track' as const, label: 'On a track' },
  { id: 'pavement' as const, label: 'On pavement' },
  { id: 'treadmill' as const, label: 'On a treadmill' },
  { id: 'trail' as const, label: 'On a trail' },
];

export const RunningScreen: React.FC<RunningScreenProps> = ({
  navigation,
}) => {
  const { user } = useAuth();
  const [selectedLocation, setSelectedLocation] = useState<RunningLocation>(null);
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      ...
      console.log('RunningScreen - Loading saved data for user:', user.id);
      const result = await profileService.getOnboardingStatus(user.id);

      if (result.success && result.profile?.onboarding_data?.running?.location) {
        console.log('RunningScreen - Pre-filling with saved location');
        setSelectedLocation(result.profile.onboarding_data.running.location);
      }
  ...
  const handleNext = async () => {
    ...
    setIsSaving(true);
    console.log('RunningScreen - Saving progress with location:', selectedLocation);
    ...
    if (saveResult.success) {
      console.log('RunningScreen - Progress saved, navigating to style');
      navigation.navigate('RunningStyle');
    } else {
      console.log('RunningScreen - Error saving progress:', saveResult.error);
      Alert.alert(
        'Error',
